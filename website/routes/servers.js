...
app.get('/', async (req,res) => {
	if (!req.isAuthenticated()) return res.redirect('/login?redirect=' + encodeURIComponent('/servers'));
...
app.get('/:id/dashboard', async (req,res) => {
  ...
  if(!req.isAuthenticated()) return res.redirect('/login?redirect=' + encodeURIComponent(`/${id}/dashboard`))
