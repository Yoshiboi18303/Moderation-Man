...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(!interaction.member.permissions.has(Permissions.FLAGS.BAN_MEMBERS)) return await interaction.reply({ content: "You can't run this command!", ephemeral: true })
    ...
    if(user.id == client.user.id) return await interaction.reply({ content: 'You can\'t ban the client!', ephemeral: true })
    if(user.id == interaction.user.id) return await interaction.reply({ content: 'You can\'t ban yourself!', ephemeral: true })
