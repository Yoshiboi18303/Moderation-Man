...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName("user").setDescription("Select a user to use for the image").setRequired(false))
    .addBooleanOption(option => option.setName("is_impostor").setDescription("Is this user the Impostor?").setRequired(false)),
  // These are comments
  async execute(interaction) {
    ...
    var link = `https://some-random-api.ml/premium/amongus?avatar=${user.displayAvatarURL({ format: 'png', dynamic: false, size: 512 })}&key=${process.env.KEY}&username=${name}&imposter=${is_impostor}`
    var name = ""
    if(is_impostor) {
      name = "impostor.gif"
    } else {
