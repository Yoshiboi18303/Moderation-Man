...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const userArray = [
      ...
      `**❯ Time Created:** ${utc(user.createdTimestamp).format("LT")} - ${utc(user.createdTimestamp).format("LL")} | ${utc(user.createdTimestamp).fromNow()}`,
      `\u200b`
    ]
    const memberArray = [
      `**❯ Highest Role:** ${member.roles.highest.id === interaction.guild.id ? 'None' : member.roles.highest.name}`,
      `**❯ Joined Server At:** ${utc(member.joinedAt).format("LL LTS")} | ${utc(member.joinedAt).fromNow()}`,
