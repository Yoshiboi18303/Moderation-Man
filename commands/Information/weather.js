...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName("location").setDescription("The location to search for").setRequired(true)),
  async execute(interaction) {
    ...
    weather.find({ search: location }, async (error, result) => {
      if(error) {
        await interaction.reply({ content: `Something went wrong! Please DM \`Yoshiboi18303#4045\`!`, ephemeral: true })
        throw error;
      } else if(result == undefined || result.length == 0) {
        await interaction.reply({ content: "Invalid location.", ephemeral: true })
      } else {
        ...
        const weather_embed = new MessageEmbed()
          ...
          .setTimestamp()
        await interaction.editReply({ embeds: [weather_embed], ephemeral: true })
      }
    })
  }
}