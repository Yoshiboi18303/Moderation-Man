...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName("type").setDescription("The optional type of status").setRequired(false).addChoice("playing", "PLAYING").addChoice("listening", "LISTENING").addChoice("competing", "COMPETING").addChoice("watching", "WATCHING")),
  async execute(interaction) {
    ...
    await interaction.reply({ content: `Setting status... ${wait}`, ephemeral: true })
    await hold(3500)
    await client.user.setActivity(`${status}`, {
      type: type
    })
    await interaction.editReply({ content: `${yes} - Successfully set my status to ${status} with the \`${type}\` type!`, ephemeral: true })
  }
}