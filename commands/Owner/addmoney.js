...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName("user").setDescription("The user to add money to").setRequired(false)),
  async execute(interaction) {
    ...
    if(amount < 1) return await interaction.editReply({ content: "You need to add at least 1 coin!", ephemeral: true })
    ...
    Profiles.findOne({ id: user.id }, async (err, data) => {
      ...
      if(!data) {
        ...
        const success_embed = new MessageEmbed()
          ...
          .setTitle("Success!")
          .setDescription(`Successfully added $${amount} coins to ${username_string_2} wallet!`)
