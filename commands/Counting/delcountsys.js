...
module.exports = {
  ...
  async execute(interaction) {
    if(!interaction.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return await interaction.reply({ content: "You can't use this command due to poor permissions!" })
    CountingSystems.findOne({ guild: interaction.guild.id }, async (err, data) => {
      if(err) throw err;
      if(!data) {
    ...
    await interaction.reply({
      content: "You are about to delete your counting system, are you sure you want to complete this?\n\n**This can be reset by running `/setupcountsys`**",
