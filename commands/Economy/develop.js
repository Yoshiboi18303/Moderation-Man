...
module.exports = {
  ...
  async execute(interaction) {
    Profiles.findOne({ id: interaction.user.id }, async (err, data) => {
      ...
      if(!data) {
      ...
      } else if(data.inventory.items.computers == 0 || typeof data.inventory.items.computers == 'undefined') {
        const bad_item_count_embed = new MessageEmbed()
          .setColor(colors.red)
          .setTitle("Error")
          .setDescription("You don't have any computers!\nPlease ask `Yoshiboi18303#4045` for one!")
          .setFooter(`${interaction.user.username} needs a computer lmao.`, interaction.user.displayAvatarURL({ dynamic: true, size: 32 }))
          .setTimestamp()
        ...
        const success_embed = new MessageEmbed()
          ...
          .setTitle("Success!")
          .setDescription(`You have successfully developed a ${thing} and earned ${earnings} coins after a few days!\nSadly.. the computer you were using blew up shortly after the release...`)
