  ...
  async execute(interaction) {
    Profiles.findOne({ id: interaction.user.id }, async (err, data) => {
      ...
      if (!data) {
        ...
        data.inventory.items.fishing_rods == 0 ||
        typeof data.inventory.items.fishing_rods == "undefined"
      ) {
        const bad_item_count_embed = new MessageEmbed()
          ...
          .setFooter(
            `${interaction.user.username} needs a fishing rod lmao.`,
            interaction.user.displayAvatarURL({ dynamic: true, size: 32 })
          )
          .setTimestamp();
        return await interaction.reply({
        ...
        var fish_caught =
          fishes_to_catch[Math.floor(Math.random() * fishes_to_catch.length)];
        ...
        const success_embed = new MessageEmbed()
          ...
          .setFooter(
            `${interaction.user.username} finished fishing!`,
