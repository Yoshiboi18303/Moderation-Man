...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(option => option.setName("money").setDescription("Type in the amount of money to withdraw").setRequired(true)),
  async execute(interaction) {
    ...
    var profile = Profiles.findOne({ id: interaction.user.id }, async (err, data) => {
      ...
      if(!data) {
        ...
        if(money_to_withdraw > vc) return await interaction.reply({ content: `There's not enough money in the vault to complete this transaction!\nThe max amount of money to withdraw is ${vc}` })
        ...
        const success_embed = new MessageEmbed()
          ...
          .setTitle("Success")
          .setDescription(`You have sucessfully withdrawn ${money_to_withdraw} coins from your vault!\nYou now have ${vc_minus} coins in your vault!`)
