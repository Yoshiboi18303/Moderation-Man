...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(option => option.setName("money").setDescription("Type in an amount of money to deposit").setRequired(true)),
  async execute(interaction) {
    var money_to_deposit = interaction.options.getInteger("money")
    if(money_to_deposit == 0) return await interaction.reply({ content: "You need to deposit at least 1 coin!", ephemeral: true })
    var profile = Profiles.findOne({ id: interaction.user.id }, async (err, data) => {
      ...
      if(!data) {
        ...
        const success_embed = new MessageEmbed()
          ...
          .setTitle("Success")
          .setDescription(`You have sucessfully deposited ${money_to_deposit} coins into your vault!\nYou now have ${vc_plus} coins in your vault, which means you have ${vm - vc_plus} coins left until max capacity!`)
