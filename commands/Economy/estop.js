  ...
  async execute(interaction) {
    Profiles.findOne({ id: interaction.user.id }, async (err, data) => {
      ...
      if(!data) {
        ...
        await interaction.reply({ content: "Are you sure you want to delete **ALL** your data?\nThis cannot be undone!", ephemeral: true, components: [row] })
        ...
        collector.on("end", async (collection) => {
          if(collection.first()?.customId === 'delete-confirm') {
            await interaction.editReply({ content: "Deleting data...", ephemeral: true, components: [] })
            data = await Profiles.findOneAndDelete({ id: interaction.user.id })
            await wait(5000)
            data.save()
            await interaction.editReply({ content: "Data deleted!", ephemeral: true })
          } else if(collection.first()?.customId === 'delete-cancel') {
            await interaction.editReply({ content: "Data deletion cancelled.", ephemeral: true, components: [] })
          }
        })
      }
    })
