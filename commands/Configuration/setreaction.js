...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (channel.type != "GUILD_TEXT")
      return await interaction.reply({
        content:
          "Invalid Channel Type. The channel needs to be of type `GUILD_TEXT` (Text)",
        ephemeral: true,
      });
    await interaction.deferReply({ ephemeral: true });
    Channels.findOne(
      { guild: interaction.guild.id, channelType: "REACTION" },
      async (err, data) => {
        if (err) throw err;
        if (!data) {
          data = new Channels({
          ...
          await interaction.editReply({
            content: `Successfully set the Reaction Roles channel to ${channel}!`,
            ephemeral: true,
          });
        } else {
          ...
          await interaction.editReply({
            content: `Successfully changed the Reaction Roles to ${channel}!`,
            ephemeral: true,
          });
        }
      }
