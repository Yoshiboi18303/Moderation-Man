...
module.exports = class BotError extends Error {
  constructor(error_text, error, critical) {
    ...
    if (!this.critical) throw new Error("Critical error option can't be left out!")
    if (typeof this.error_text != "string")
      throw new Error(
        `Expected error text type to be a type of string, got ${typeof this
          .error_text} instead.`
      );
    if (typeof this.critical != "boolean") throw new Error(`Expected critical type to be a type of boolean, got ${typeof this.error_text} instead.`)
